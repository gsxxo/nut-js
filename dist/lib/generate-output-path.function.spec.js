...
describe("generate-output-path", () => {
    it("should default to a PNG file without pre- or postfix in the current directory", () => {
        ...
        const expectedPath = path_1.join(process_1.cwd(), `${filename}${ext}`);
        // WHEN
        const result = generate_output_path_function_1.generateOutputPath(filename);
        // THEN
    ...
    it("should should allow to add a prefix to the filename", () => {
        ...
        const pre = "foo_";
        const ext = file_type_enum_1.FileType.PNG;
        const expectedPath = path_1.join(process_1.cwd(), `${pre}${filename}${ext}`);
        // WHEN
        const result = generate_output_path_function_1.generateOutputPath(filename, { prefix: pre });
        // THEN
        expect(result).toEqual(expectedPath);
    });
    it("should should allow to add a postfix to the filename", () => {
        ...
        const post = "_bar";
        const ext = file_type_enum_1.FileType.PNG;
        const expectedPath = path_1.join(process_1.cwd(), `${filename}${post}${ext}`);
        // WHEN
        const result = generate_output_path_function_1.generateOutputPath(filename, { postfix: post });
        // THEN
        expect(result).toEqual(expectedPath);
    });
    it("should should allow to add both a prefix and a postfix to the filename", () => {
        ...
        const post = "_bar";
        const ext = file_type_enum_1.FileType.PNG;
        const expectedPath = path_1.join(process_1.cwd(), `${pre}${filename}${post}${ext}`);
        // WHEN
        const result = generate_output_path_function_1.generateOutputPath(filename, {
            postfix: post,
            prefix: pre,
    ...
    it("should should allow to configure the file path", () => {
        ...
        const ext = file_type_enum_1.FileType.PNG;
        const expectedPath = path_1.join(filepath, `${filename}${ext}`);
        // WHEN
        const result = generate_output_path_function_1.generateOutputPath(filename, {
            path: filepath,
        });
