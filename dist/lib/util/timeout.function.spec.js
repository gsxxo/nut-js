...
describe("timeout", () => {
    it("should timeout after maxDuration if action rejects", async () => {
        ...
        try {
            await timeout_function_1.timeout(updateInterval, maxDuration, action);
        }
        catch (e) {
    ...
    it("should retry until action succeeds", async () => {
        ...
        // WHEN
        const result = await timeout_function_1.timeout(updateInterval, maxDuration, action);
        const end = Date.now();
        // THEN
    ...
    it("should fail if action does not resolve within timeout", async () => {
        ...
        // WHEN
        const SUT = () => timeout_function_1.timeout(updateInterval, maxDuration, action);
        // THEN
    ...
    it("should be externally abortable", async () => {
        ...
        // WHEN
        const SUT = timeout_function_1.timeout(updateInterval, maxDuration, action, { signal });
