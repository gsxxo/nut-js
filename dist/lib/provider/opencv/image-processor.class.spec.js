...
describe("ImageProcessor", () => {
    it("should allow to create a cv.Mat from an Image with alpha channel, alpha channel is dropped", async () => {
        ...
        const imagePath = path_1.resolve(__dirname, "./__mocks__/alpha_channel.png");
        const image = await imageReader.load(imagePath);
        // WHEN
        const mat = await image_processor_class_1.fromImageWithAlphaChannel(image);
        // THEN
    ...
    it("should allow to create a cv.Mat from an Image without alpha channel", async () => {
        // GIVEN
        const imageReader = new image_reader_class_1.ImageReader();
        const imagePath = path_1.resolve(__dirname, "./__mocks__/mouse.png");
        const image = await imageReader.load(imagePath);
        // WHEN
        const mat = await image_processor_class_1.fromImageWithoutAlphaChannel(image);
        // THEN
...
describe("ImageProcessor with ROI", () => {
    it("negative left or top values are updated to 0", async () => {
        ...
        const image = await imageReader.load(imagePath);
        // WHEN
        const mat = await image_processor_class_1.fromImageWithoutAlphaChannel(image, new region_class_1.Region(-100, -100, 10, 10));
        // THEN
        expect(image.hasAlphaChannel).toBeFalsy();
        expect(mat.channels).toEqual(3);
    ...
    it("values bigger than the input are updated to width and height", async () => {
        ...
        const image = await imageReader.load(imagePath);
        // WHEN
        const mat = await image_processor_class_1.fromImageWithoutAlphaChannel(image, new region_class_1.Region(10, 10, image.width * 2, image.height * 2));
        // THEN
        expect(image.hasAlphaChannel).toBeFalsy();
        expect(mat.channels).toEqual(3);
